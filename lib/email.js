...
export async function sendBookingConfirmation(order) {
  ...
  const phone = brand.phone || '07849 533923';
  const website = brand.website || 'superdrycleaners.co';
  const name = brand.name || 'SuperDryCleaners';
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <div style="padding: 30px 20px; background: #f4f6fb;">
        ...
        <p style="color: #666; font-size: 14px;">If you need to make changes, reply to this email or WhatsApp us at ${phone}.</p>
      </div>
      <div style="background: #0a1f44; padding: 16px; text-align: center;">
        <p style="color: #aaa; font-size: 12px; margin: 0;">${name} · Leicester · ${website}</p>
      </div>
    </div>
  `;
...
export async function sendPickupConfirmation(order) {
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <div style="background: #0a1f44; padding: 16px; text-align: center;">
        <p style="color: #aaa; font-size: 12px; margin: 0;">${name} · ${phone} · ${website}</p>
      </div>
    </div>
  `;
