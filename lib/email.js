...
export async function sendEmail({ to, subject, html, text, attachments }) {
  ...
  try {
    await transporter.sendMail({
      from: `"SuperDryCleaners" <${smtpEmail}>`,
      to,
      subject,
      html,
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <div style="background: #0a1f44; padding: 20px; text-align: center;">
        <h1 style="color: #fff; margin: 0; font-size: 22px;">SuperDryCleaners</h1>
      ...
      <div style="padding: 30px 20px; background: #f4f6fb;">
        ...
        <p style="color: #666; font-size: 14px;">If you need to make changes, reply to this email or WhatsApp us at 07849 533923.</p>
      </div>
      <div style="background: #0a1f44; padding: 16px; text-align: center;">
        <p style="color: #aaa; font-size: 12px; margin: 0;">SuperDryCleaners · Leicester · superdrycleaners.co</p>
      </div>
    </div>
  `;
...
export async function sendPickupConfirmation(order) {
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <div style="background: #0a1f44; padding: 16px; text-align: center;">
        <p style="color: #aaa; font-size: 12px; margin: 0;">SuperDryCleaners · 07849 533923 · superdrycleaners.co</p>
      </div>
    </div>
  `;
...
export async function sendInvoiceEmail(order, pdfBuffer) {
  ...
  return await sendEmail({
    to: order.email,
    subject: `Invoice ${order.id} — SuperDryCleaners`,
    html,
    attachments: [
      {
