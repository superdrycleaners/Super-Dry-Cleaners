/**
 * Orders / collection-requests data-access layer.
 *
 * All order reads and writes flow through this module. Backend: Supabase
 * Postgres when available, otherwise falls back to an in-memory store
 * (for local dev without network access to Supabase).
 */

import { supabase } from '@/lib/supabase';

/**
 * In-memory fallback store for when Supabase is unreachable/unconfigured.
 * @type {import('./orders').Order[]}
 */
let memoryStore = [];

/**
 * @typedef {'pending'|'confirmed'|'collected'|'in_progress'|'ready'|'delivered'|'cancelled'} OrderStatus
 */

/**
 * @typedef {object} OrderItem
 * @property {string} name - Item/service name.
 * @property {number} qty - Quantity.
 * @property {number} price - Price per unit in pence.
 */

/**
 * @typedef {object} Order
 * @property {string} id - Unique order id.
 * @property {string} name - Customer full name.
 * @property {string} email - Customer email.
 * @property {string} phone - Customer phone.
 * @property {string} service - Requested service.
 * @property {OrderItem[]} [items] - Line items with prices.
 * @property {number} [total] - Total in pence.
 * @property {string} [couponCode] - Applied offer code.
 * @property {string} [discountType] - 'percent' or 'fixed'.
 * @property {number} [discountValue] - Discount amount (percent or pence).
 * @property {string} [discountLabel] - Human-readable offer name.
 * @property {string} address1 - Address line 1.
 * @property {string} [address2] - Address line 2.
 * @property {string} city - City / borough.
 * @property {string} postcode - Postcode.
 * @property {string} [notes] - Access notes.
 * @property {number|null} lat - Pickup latitude from the map pin.
 * @property {number|null} lng - Pickup longitude from the map pin.
 * @property {string} date - Requested collection date (YYYY-MM-DD).
 * @property {string} slot - Requested time slot.
 * @property {OrderStatus} status - Current status.
 * @property {boolean} [emailSent] - True if booking confirmation email was sent successfully.
 * @property {string} createdAt - ISO timestamp of creation.
 */

/** Ordered pipeline of statuses used by the dashboard. */
export const ORDER_STATUSES = [
  'pending',
  'confirmed',
  'collected',
  'in_progress',
  'ready',
  'delivered',
  'cancelled',
];

/** Human-friendly labels for each status. */
export const STATUS_LABELS = {
  pending: 'Pending',
  confirmed: 'Confirmed',
  collected: 'Collected',
  in_progress: 'In progress',
  ready: 'Ready',
  delivered: 'Delivered',
  cancelled: 'Cancelled',
};

/**
 * Generate a sequential order ID formatted as `ORD-xxxx`.
 * Padded with 4 digits starting from 0001 (or next highest number if existing orders exist).
 *
 * @returns {string} Unique sequential order ID.
 */
export function generateOrderId() {
  const existingIds = new Set(memoryStore.map((o) => o.id));

  let maxNum = -1;
  for (const id of existingIds) {
    const match = /^ORD-(\d+)$/i.exec(id);
    if (match) {
      const num = parseInt(match[1], 10);
      if (!isNaN(num) && num > maxNum) {
        maxNum = num;
      }
    }
  }

  let nextNum = maxNum >= 0 ? maxNum + 1 : 1;
  let candidate = `ORD-${String(nextNum).padStart(4, '0')}`;

  while (existingIds.has(candidate)) {
    nextNum++;
    candidate = `ORD-${String(nextNum).padStart(4, '0')}`;
  }

  return candidate;
}

/**
 * Async generator that checks both Supabase and memoryStore for existing order IDs
 * to guarantee no duplicates and return the next sequential `ORD-xxxx` ID.
 *
 * @returns {Promise<string>} Next unique sequential order ID.
 */
export async function generateNextOrderId() {
  const existingIds = new Set(memoryStore.map((o) => o.id));

  if (supabase) {
    try {
      const { data } = await supabase.from('orders').select('id');
      if (data) {
        for (const row of data) {
          if (row && row.id) existingIds.add(row.id);
        }
      }
    } catch {
      /* silent fallback */
    }
  }

  let maxNum = -1;
  for (const id of existingIds) {
    const match = /^ORD-(\d+)$/i.exec(id);
    if (match) {
      const num = parseInt(match[1], 10);
      if (!isNaN(num) && num > maxNum) {
        maxNum = num;
      }
    }
  }

  let nextNum = maxNum >= 0 ? maxNum + 1 : 1;
  let candidate = `ORD-${String(nextNum).padStart(4, '0')}`;

  while (existingIds.has(candidate)) {
    nextNum++;
    candidate = `ORD-${String(nextNum).padStart(4, '0')}`;
  }

  return candidate;
}

/**
 * Map a Supabase row to the Order shape used by the app.
 * Handles the snake_case → camelCase conversion.
 *
 * @param {object} row - Supabase row.
 * @returns {Order} App-shaped order.
 */
function rowToOrder(row) {
  return {
    id: row.id,
    name: row.name,
    email: row.email,
    phone: row.phone,
    service: row.service,
    items: row.items || [],
    total: row.total || 0,
    couponCode: row.coupon_code || row.couponCode || null,
    discountType: row.discount_type || row.discountType || null,
    discountValue: row.discount_value || row.discountValue || 0,
    discountLabel: row.discount_label || row.discountLabel || null,
    address1: row.address1,
    address2: row.address2 || '',
    city: row.city,
    postcode: row.postcode,
    notes: row.notes || '',
    quantity: row.quantity || '',
    deliveryPreference: row.delivery_preference || row.deliveryPreference || '',
    lat: row.lat,
    lng: row.lng,
    date: row.date,
    slot: row.slot,
    status: row.status,
    emailSent: row.email_sent ?? row.emailSent ?? false,
    createdAt: row.created_at || row.createdAt,
  };
}

/**
 * List all orders, newest first.
 *
 * @param {object} [options] - Filter options.
 * @param {OrderStatus} [options.status] - Restrict to a single status.
 * @returns {Promise<Order[]>} Matching orders.
 */
export async function listOrders({ status } = {}) {
  if (!supabase) {
    let result = [...memoryStore];
    if (status) result = result.filter((o) => o.status === status);
    result.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
    return result;
  }

  let query = supabase
    .from('orders')
    .select('*')
    .order('created_at', { ascending: false });

  if (status) {
    query = query.eq('status', status);
  }

  try {
    const { data, error } = await query;
    if (error) {
      console.error('[Orders] Failed to list orders from Supabase, using in-memory fallback:', error.message);
      // Graceful fallback: return in-memory store so UI is never blank
      let fallback = [...memoryStore];
      if (status) fallback = fallback.filter((o) => o.status === status);
      fallback.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
      return fallback;
    }
    return (data || []).map(rowToOrder);
  } catch (err) {
    console.error('[Orders] Network error fetching orders, using in-memory fallback:', err.message);
    let fallback = [...memoryStore];
    if (status) fallback = fallback.filter((o) => o.status === status);
    fallback.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
    return fallback;
  }
}

/**
 * Fetch a single order by id.
 *
 * @param {string} id - Order id.
 * @returns {Promise<Order|null>} The order, or null if not found.
 */
export async function getOrder(id) {
  if (!supabase) {
    return memoryStore.find((o) => o.id === id) || null;
  }

  try {
    const { data, error } = await supabase
      .from('orders')
      .select('*')
      .eq('id', id)
      .single();

    if (error) {
      if (error.code === 'PGRST116') return null;
      console.error('[Orders] Failed to get order from Supabase, using in-memory fallback:', error.message);
      return memoryStore.find((o) => o.id === id) || null;
    }
    return data ? rowToOrder(data) : null;
  } catch (err) {
    console.error('[Orders] Network error fetching order, using in-memory fallback:', err.message);
    return memoryStore.find((o) => o.id === id) || null;
  }
}

/**
 * Create a new order from validated booking input.
 *
 * @param {Omit<Order,'id'|'status'|'createdAt'> & { id?: string }} input - Validated fields.
 * @returns {Promise<Order>} The created order.
 */
export async function createOrder(input) {
  const id = input.id || (await generateNextOrderId());

  if (!supabase) {
    const order = {
      id, ...input,
      status: 'pending',
      emailSent: input.emailSent || false,
      createdAt: new Date().toISOString(),
      items: input.items || [],
      total: input.total || 0,
      couponCode: input.couponCode || null,
      discountType: input.discountType || null,
      discountValue: input.discountValue || 0,
      discountLabel: input.discountLabel || null,
    };
    memoryStore.unshift(order);
    return { ...order };
  }

  const row = {
    id,
    name: input.name,
    email: input.email,
    phone: input.phone,
    service: input.service,
    address1: input.address1,
    address2: input.address2 || '',
    city: input.city,
    postcode: input.postcode,
    notes: input.notes || '',
    quantity: input.quantity || '',
    delivery_preference: input.deliveryPreference || '',
    lat: input.lat || null,
    lng: input.lng || null,
    date: input.date,
    slot: input.slot,
    status: 'pending',
    email_sent: input.emailSent || false,
    items: input.items || [],
    total: input.total || 0,
    coupon_code: input.couponCode || null,
    discount_type: input.discountType || null,
    discount_value: input.discountValue || 0,
    discount_label: input.discountLabel || null,
    created_at: new Date().toISOString(),
  };

  try {
    const { data, error } = await supabase
      .from('orders')
      .insert(row)
      .select()
      .single();

    if (error) {
      console.error('[Orders] Failed to create order in Supabase, saving to in-memory fallback:', error.message);
      // Save to memory store so it appears in the admin panel immediately
      const fallbackOrder = { ...row, createdAt: row.created_at };
      memoryStore.unshift(fallbackOrder);
      return fallbackOrder;
    }
    // Also cache in memory store so it survives brief Supabase hiccups
    const created = rowToOrder(data);
    memoryStore.unshift(created);
    return created;
  } catch (err) {
    console.error('[Orders] Network error creating order, saving to in-memory fallback:', err.message);
    const fallbackOrder = { ...row, createdAt: row.created_at };
    memoryStore.unshift(fallbackOrder);
    return fallbackOrder;
  }
}

/**
 * Update an order's status.
 *
 * @param {string} id - Order id.
 * @param {OrderStatus} status - New status; must be a known value.
 * @returns {Promise<Order|null>} The updated order, or null if not found.
 */
export async function updateOrderStatus(id, status) {
  if (!ORDER_STATUSES.includes(status)) {
    throw new Error(`Invalid status: ${status}`);
  }

  if (!supabase) {
    const order = memoryStore.find((o) => o.id === id);
    if (!order) return null;
    order.status = status;
    return { ...order };
  }

  const { data, error } = await supabase
    .from('orders')
    .update({ status })
    .eq('id', id)
    .select()
    .single();

  if (error) {
    if (error.code === 'PGRST116') return null;
    console.error('[Orders] Failed to update order status:', error.message);
    return null;
  }
  return data ? rowToOrder(data) : null;
}

/**
 * Update an order's invoice line items and total.
 *
 * @param {string} id - Order id.
 * @param {OrderItem[]} items - New line items.
 * @param {number} total - New total in pence.
 * @returns {Promise<Order|null>} The updated order, or null if not found.
 */
export async function updateOrderInvoice(id, items, total) {
  if (!supabase) {
    const order = memoryStore.find((o) => o.id === id);
    if (!order) return null;
    order.items = items;
    order.total = total;
    return { ...order };
  }

  const { data, error } = await supabase
    .from('orders')
    .update({ items, total })
    .eq('id', id)
    .select()
    .single();

  if (error) {
    if (error.code === 'PGRST116') return null;
    console.error('[Orders] Failed to update invoice:', error.message);
    return null;
  }
  return data ? rowToOrder(data) : null;
}

/**
 * Aggregate counts for the dashboard summary cards.
 *
 * @returns {Promise<{ total: number, byStatus: Record<string, number> }>}
 */
export async function getOrderStats() {
  if (!supabase) {
    const byStatus = Object.fromEntries(ORDER_STATUSES.map((s) => [s, 0]));
    for (const o of memoryStore) {
      if (o.status in byStatus) byStatus[o.status] += 1;
    }
    return { total: memoryStore.length, byStatus };
  }

  try {
    const { data, error } = await supabase
      .from('orders')
      .select('status');

    if (error) {
      console.error('[Orders] Failed to get order stats from Supabase, using in-memory fallback:', error.message);
      const byStatus = Object.fromEntries(ORDER_STATUSES.map((s) => [s, 0]));
      for (const o of memoryStore) {
        if (o.status in byStatus) byStatus[o.status] += 1;
      }
      return { total: memoryStore.length, byStatus };
    }

    const byStatus = Object.fromEntries(ORDER_STATUSES.map((s) => [s, 0]));
    for (const row of data || []) {
      if (row.status in byStatus) byStatus[row.status] += 1;
    }
    return { total: (data || []).length, byStatus };
  } catch (err) {
    console.error('[Orders] Network error fetching order stats, using in-memory fallback:', err.message);
    const byStatus = Object.fromEntries(ORDER_STATUSES.map((s) => [s, 0]));
    for (const o of memoryStore) {
      if (o.status in byStatus) byStatus[o.status] += 1;
    }
    return { total: memoryStore.length, byStatus };
  }
}

/**
 * Update an order's email_sent status flag.
 *
 * @param {string} id - Order id.
 * @param {boolean} emailSent - True if email sent successfully, false otherwise.
 * @returns {Promise<Order|null>} The updated order, or null if not found.
 */
export async function updateOrderEmailSent(id, emailSent = true) {
  const isSent = Boolean(emailSent);

  if (!supabase) {
    const order = memoryStore.find((o) => o.id === id);
    if (!order) return null;
    order.emailSent = isSent;
    return { ...order };
  }

  try {
    const { data, error } = await supabase
      .from('orders')
      .update({ email_sent: isSent })
      .eq('id', id)
      .select()
      .single();

    if (error) {
      console.error('[Orders] Failed to update email_sent status in Supabase:', error.message);
      const order = memoryStore.find((o) => o.id === id);
      if (order) order.emailSent = isSent;
      return order || null;
    }

    const updated = rowToOrder(data);
    const memOrder = memoryStore.find((o) => o.id === id);
    if (memOrder) memOrder.emailSent = isSent;
    return updated;
  } catch (err) {
    console.error('[Orders] Network error updating email_sent status:', err.message);
    const order = memoryStore.find((o) => o.id === id);
    if (order) order.emailSent = isSent;
    return order || null;
  }
}

