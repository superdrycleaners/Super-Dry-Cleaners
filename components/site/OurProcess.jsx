...
function renderIcon(iconKey) {
  ...
  return ICONS[iconKey] || ICONS.search;
}
...
const DEFAULT_STEPS = [
  ...
  },
  {
    ...
    desc: "Every garment is carefully finished and returned ready to wear.",
    icon: "star"
  }
];
...
export default function OurProcess({ data }) {
  ...
  const eyebrow = data?.eyebrow || brand?.name || 'SUPER DRY CLEANERS';
  const title = data?.title || 'Our Cleaning Process';
  const intro = data?.intro || 'Experience professional garment care. Our modern cleaning process is designed to provide excellent cleaning results while being gentler on fabrics and the environment.';
  const steps = Array.isArray(data?.steps) && data.steps.length > 0 ? data.steps : DEFAULT_STEPS;
    ...
    <section className="section" id="eco-process" style={{ background: 'var(--mist)' }}>
      <div className="container">
        ...
        <div className="grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '2rem' }}>
          {steps.map((step, idx) => (
            <Reveal className="card" key={step.id || idx} style={{ display: 'flex', flexDirection: 'column', padding: '2.5rem 2rem', position: 'relative', overflow: 'hidden' }}>
